...
def collect_season_matchups(league, up_to_week=None):
    ...
    for week in range(1, limit + 1):
        ...
        for m in box_scores:
            ...
            # Only the real championship bracket counts as "made the playoffs" —
            # ESPN also tags consolation/losers-bracket games (which non-playoff
            # teams play in the same final weeks) with a non-None matchup_type,
            # so checking "not NONE" alone incorrectly counted those too and
            # made nearly every manager's playoff-appearance count identical.
            is_playoff = str(matchup_type).upper() == "WINNERS_BRACKET" if matchup_type else False
...
def build_historical_seasons_and_matchups(league_id, swid, espn_s2, start_year, end_year):
    ...
    for year in range(start_year, end_year + 1):
        try:
            ...
            playoff_team_count = getattr(yr_league.settings, "playoff_team_count", None)
            seasons.append({
                "year": year, "teams": build_manager_index(yr_league),
            ...
            print(f"  {year}: {games_this_year} games, playoff_team_count={playoff_team_count}{flag}")
...
def build_head_to_head_log(all_matchups, manager_a, manager_b):
    ...
    games.sort(key=lambda g: (g["year"], g["week"]))  # oldest first — the full history of the rivalry, front to back
...
PLAYOFF_CUTOFF = 6  # spots 1-6 in final regular-season standing = "made the playoffs", for our purposes


def build_playoff_stats(historical_seasons):
    """
    Real playoff-appearance stats across every season on file, computed
    from final standings using a FIXED cutoff: standing 1-6 = made the
    playoffs that year, standing 7+ = didn't. This intentionally does
    NOT depend on ESPN's per-season playoff_team_count field, which
    turned out to be unavailable/unreliable for enough seasons that an
    earlier version of this function (which only counted seasons where
    that field was present) still badly undercounted everyone. Standing
    itself is reliably available for every team, every season, so a
    fixed cutoff applies cleanly across all 16 seasons with nothing
    skipped.

    Returns:
      - most_appearances / least_appearances: manager with the most/
        fewest playoff-season counts (out of all completed seasons).
      - longest_drought: manager with the longest CURRENT drought,
        counting back from the most recent completed season.
    """
    completed = [s for s in historical_seasons if s.get("teams") and not s.get("error")]
    if not completed:
        return {}

    years_sorted = sorted(s["year"] for s in completed)
    all_managers = set(t["manager"] for s in completed for t in s["teams"] if t.get("manager"))

    playoff_years_by_manager = defaultdict(set)
    for s in completed:
        for t in s["teams"]:
            if t.get("manager") and t.get("standing") and t["standing"] <= PLAYOFF_CUTOFF:
                playoff_years_by_manager[t["manager"]].add(s["year"])

    total_seasons = len(years_sorted)
    appearance_counts = {mgr: len(playoff_years_by_manager.get(mgr, set())) for mgr in all_managers}
    print("  Playoff appearance counts:", dict(sorted(appearance_counts.items(), key=lambda kv: -kv[1])))
    most_appearances_mgr = max(appearance_counts, key=lambda m: appearance_counts[m]) if appearance_counts else None
    least_appearances_mgr = min(appearance_counts, key=lambda m: appearance_counts[m]) if appearance_counts else None
    ...
    if least_appearances_mgr:
        result["least_appearances"] = {
            "manager": least_appearances_mgr,
            "count": appearance_counts[least_appearances_mgr],
            "total_seasons": total_seasons,
        }
...
def find_latest_completed_week(league):
    """
    Finds the most recent week with real (non-zero) scores — Weekly
    Awards only cares about "this week's" box scores, not the full
    season. Scans backward from the league's current week (falling back
    to MAX_WEEKS_TO_SCAN if that's not available) so it lands on the
    latest week that's actually been played, not an upcoming empty one.
    Returns (week_number, box_scores), or (None, []) if nothing's been
    played yet this season.
    """
    start_week = getattr(league, "current_week", None) or MAX_WEEKS_TO_SCAN
    for week in range(start_week, 0, -1):
        try:
            box_scores = league.box_scores(week)
        except Exception:
            continue
...
def main():
    ...
    playoff_stats = build_playoff_stats(historical_seasons)
