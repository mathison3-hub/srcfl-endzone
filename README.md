...
- **Trophy Case & Playoff History** — built from ESPN's final "standing" field
  per season, preferring `final_standing` (the actual post-season/playoff-aware
  rank) over the plain `standing` field where ESPN has it populated — an earlier
  version used `standing` alone, which mismatched the real champion for at least
  one season (2024). Worth an eyeball-check across all years after any refresh,
  since this was only caught manually.
